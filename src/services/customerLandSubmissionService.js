...
export function calculateLandTotalPrice({
  ...
  const normalizedBrokerageAmount =
    validateNonNegativeNumber(
      brokerageAmount,
      "السعي"
    );
...
export async function getMyFinancedLandSubmission(
  customerFileId
) {
  const validatedCustomerFileId =
    validateUuid(
  ...
  const { data, error } =
    await supabase.rpc(
      "customer_get_my_financed_land_submission",
      {
        p_customer_file_id:
          validatedCustomerFileId,
      }
...
export async function createLandDeedSignedUrl(
  storagePath,
