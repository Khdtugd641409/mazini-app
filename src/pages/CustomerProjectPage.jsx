...
function getProjectRouteFromPath() {
  const pathParts =
...
function validateLandFormBeforeReview({
  formData,
  deedFile,
}) {
  ...
  if (
    ...
    throw new Error("أدخل مساحة الأرض.");
  }
  ...
  if (
    ...
    throw new Error("أدخل عرض الشارع.");
  }
...
function CustomerLandSubmissionFlow({
  customerFileId,
}) {
  const [formData, setFormData] =
    useState(INITIAL_LAND_FORM_DATA);
...
export default function CustomerProjectPage() {
  ...
  if (route.mode === "land") {
    return (
      <CustomerLandSubmissionFlow
        customerFileId={route.projectId}
      />
    );
  }
  ...
  return (
    <CustomerFilePage
      ...
      timeline={timeline}
      onBackToHome={handleBackToProjects}
    />
  );
}
